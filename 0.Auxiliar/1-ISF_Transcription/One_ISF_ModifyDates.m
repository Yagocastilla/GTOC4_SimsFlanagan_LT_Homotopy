function modSol = One_ISF_ModifyDates(init_arc,v_ini,initMass,nArcs,asteroidsOE,solution,pars)

    %Copy the solution
    modSol = solution;

    %Initial guess
    initGuess = zeros(nArcs-1,1);

    %Set objective function
    objFunc = @(dateModifiers)trajectoryMaxThrust(init_arc,v_ini,initMass,nArcs,asteroidsOE,solution,dateModifiers,pars);
    ...
    b = zeros(nArcs,1);
    if init_arc == 1
        b(1) = solution.Flyby_Dates(init_arc) - solution.Launch_Date;
    else
        b(1) = solution.Flyby_Dates(init_arc) - solution.Flyby_Dates(init_arc-1);
    end
    for k = 2:nArcs
        b(k) = solution.Flyby_Dates(init_arc+k-1) - solution.Flyby_Dates(init_arc+k-2);
    end
    %b(nArcs+1) = (solution.Flyby_Dates(init_arc+nArcs) - solution.Flyby_Dates(init_arc))/nArcs;

    %Set the bound on the date modifiers
    ...
    dateModifiers = fmincon(objFunc,initGuess,A,b,[],[],[],[],[],opt_options);

    %Compute the new dates
    modSol.Flyby_Dates(init_arc:init_arc+nArcs-2) = modSol.Flyby_Dates(init_arc:init_arc+nArcs-2) + dateModifiers.';
end

function maxThrust = trajectoryMaxThrust(init_arc,v_ini,initMass,nArcs,asteroidsOE,solution,dateModifiers,pars)
