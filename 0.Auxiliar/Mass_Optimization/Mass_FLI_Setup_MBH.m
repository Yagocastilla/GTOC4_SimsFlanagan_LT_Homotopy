function [impulses,opt_results] = Mass_FLI_Setup_MBH(r_ini,v_ini,r_fin,req_v_fin,ToF,n_i,pars,initSF,dv_limit,kmax,rho,max_rep)
...
[opt_solution, fout,~,fminout] = fmincon(penalObj,opt_dv_0,[],[],[],[],-bounds,bounds,constraint,opt_options);

%Variable to store current best
opt_results.best_ev = zeros(1,kmax+1);

%Variable to store constraint violation of current best
opt_results.best_constrviolation_ev = zeros(1,kmax+1);

j = 0; %Number of total local runs
rep = 0; %Number of local runs without improvement
current_best = opt_solution; %Best solution
current_best_value = fout; %Objective function's value of best solution
best_constrviolation = fminout.constrviolation;
opt_results.best_ev(1) = current_best_value; %Store the current best
opt_results.best_constrviolation_ev(1) = best_constrviolation; %Store the constrviolation history

%Monotonic basin hopping loop
while j<kmax
    %Generate the random perturbation vector
    ...
    %Generate new initial guess by perturbing the current best
    new_guess = current_best + pert;

    %Try to find a feasible solution
    [feasible_guess,~] = fmincon(@(x)0,new_guess,[],[],[],[],-bounds,bounds,constraint,opt_options);

    %Local Optimization Algorithm
    [opt_solution, fout, exitflag,fminout] = fmincon(penalObj,feasible_guess,[],[],[],[],-bounds,bounds,constraint,opt_options);

    %Update counter of local runs
    j = j + 1;

    %If a better feasible solution is find, update the current best
    if best_constrviolation <= 1e-6
        if fout < current_best_value
            if (exitflag == 1 || exitflag == 2) || fminout.constrviolation <= 1e-6
        ...
        if fminout.constrviolation < best_constrviolation
            current_best = opt_solution;
            current_best_value = fout;
            best_constrviolation = fminout.constrviolation;
            rep = 0;
    ...
    %Save current best constraint violation
    opt_results.best_constrviolation_ev(j+1) = best_constrviolation;

    %After the specified runs, if no improvement found, stop the algorithm
    if rep == max_rep
        break
    end
end
...
for k=1:size(n_dV_opt,1)
    n_dV_opt(k,:) = current_best((k-1)*3+1:k*3);
end
