function trajectory = obtain_Segment_Trajectory(r_ini,v_ini,delta_t,pars,mesh_refiner)

%{
###########################################################################
                          Obtain Segment Trajectory
                        Author:  Yago Castilla Lamas
###########################################################################

This function propagates the keplerian orbit providing the position vector
in a number of intermediate points in between the initial and final
position (the points are obtained as a linear discretization in the true
anomaly):
...
[a, e, i, RAN, Omega, ~] = Build_OE(r_ini, v_ini, pars.mu_sun);
